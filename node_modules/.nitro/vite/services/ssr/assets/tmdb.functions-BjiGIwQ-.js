...
//#endregion
//#region src/lib/tmdb.functions.ts?tss-serverfn-split
...
var getGenres = createServerFn({ method: "GET" }).inputValidator(z.object({ type: z.enum(["movie", "tv"]) })).handler(getGenres_createServerFn_handler, async ({ data }) => {
...
var searchMedia = createServerFn({ method: "GET" }).inputValidator(z.object({ query: z.string() })).handler(searchMedia_createServerFn_handler, async ({ data }) => {
