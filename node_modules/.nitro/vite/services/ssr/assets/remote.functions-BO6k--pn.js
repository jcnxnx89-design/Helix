...
import { z } from "zod";
//#region src/lib/remote.functions.ts?tss-serverfn-split
...
var claimPairing = createServerFn({ method: "POST" }).inputValidator(z.object({ code: z.string().max(16) })).handler(claimPairing_createServerFn_handler, async ({ data }) => {
...
var endPairing = createServerFn({ method: "POST" }).inputValidator(z.object({ sessionId: z.string().uuid() })).handler(endPairing_createServerFn_handler, async ({ data }) => {
	const { supabaseAdmin } = await import("./client.server-KzwUIAkW.js");
