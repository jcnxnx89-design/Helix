...
import { t as createServerRpc } from "./createServerRpc-PeZKVqZb.js";
import { z } from "zod";
//#region src/lib/sources.functions.ts?tss-serverfn-split
...
var listAllSources = createServerFn({ method: "POST" }).inputValidator(z.object({ passcode: z.string().max(200) })).handler(listAllSources_createServerFn_handler, async ({ data }) => {
...
var verifyPasscode = createServerFn({ method: "POST" }).inputValidator(z.object({ passcode: z.string().max(200) })).handler(verifyPasscode_createServerFn_handler, async ({ data }) => {
	assertAdmin(data.passcode);
	return { ok: true };
});
//#endregion
